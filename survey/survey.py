...
class survey_survey(osv.Model):
    ...
    _columns = {
        ...
        'survey_page_length':fields.integer('Survey Page Length'),
    }
    ...
    def next_page(self, cr, uid, user_input, page_id, go_back=False, context=None):
        ...
        ########### Hide Page ##############
        pages_list = []
        for page_record in survey.page_ids:
            if page_record.hide_page != True:
                pages_list.append(page_record)
        self.write(cr,uid,[survey.id],{'survey_page_length':len(pages_list)},context=context)
        #### previous Logic pages = list(enumerate(survey.page_ids))
        ###Change Logic
        pages = list(enumerate(pages_list))
        ##################### End Of Hide Page Logic ################
        # First page
        if page_id == 0:
...
class survey_page(osv.Model):
    ...
    _columns = {
        ...
        'description': fields.html('Description',
            ...
            oldname="note"),
        'is_for_kits':fields.boolean("Check If this page used for Kits"),
        'is_for_monthly_appraisal_page':fields.boolean("Check If this page used for Monthly Appraisal"),
        'hide_page':fields.boolean("Hide",help="Check this Box If you want to hide this Page"),
    }
    _defaults = {
        'sequence': 10,
        'hide_page':False,
...
class survey_question(osv.Model):
    ...
    _columns = {
        ...
        'type': fields.selection([('free_text', 'Long Text Zone'),
                ...
                ('matrix', 'Matrix'),
                ('matrix_internal','Matrix For Appraisal'),
                ('matrix_kits','Kits For First Cycle'),
                ('kits_cycle_2','Kits For Second Cycle'),
                ('matrix_monthly','Matrix For Monthly Appraisal')], 'Type of Question', size=30, required=1),
        ...
        'labels_ids_3': fields.one2many('survey.label',
            'question_id_3', 'Types of answer for Appraiser', copy=True),
        ...
        'is_for_kit_cycle_1':fields.boolean("Question for Kits Cycle 1",help="Check this box when question is related to kits Cycle 1"),
        'is_for_kit_cycle_2':fields.boolean("Question for Kits Cycle 2",help="Check this box when question is related to kits Cycle 2"),
        'is_for_monthly_appraisal':fields.boolean("Question for Monthly Appraisal",help="Check this box when question is related to Monthly Appraisal"),
        'is_for_appraiser': fields.boolean("Is question for only Appraiser",help="Check this box If this question only related to Appraiser"),
        'is_readonly': fields.boolean("Is question for ReadOnly",help="Check this box If this question is only for Read")
    }
    ...
    def validate_numerical_box(self, cr, uid, question, post, answer_tag, context=None):
        ...
        # Answer validation (if properly defined)user_input
        if answer and question.validation_required:
    ...
    def validate_simple_choice(self, cr, uid, question, post, answer_tag, context=None):
        ...
        # Empty answer to mandatory questioncomment
    ...
    def validate_matrix_internal(self, cr, uid, question, post, answer_tag, context=None):
        errors = {}
        if question.constr_mandatory:
            lines_number = len(question.labels_ids_2)
            answer_candidates = dict_keys_startswith(post, answer_tag)
            ...
            for value in answer_candidates :
                strip_value = value.split('_')
                if strip_value[3] ==  'appraisee':
                    appraisee_answer = appraisee_answer + 1
                if strip_value[3] == 'appraiser':
                    appraiser_answer = appraiser_answer + 1
            if post['group'] == 'employee':
                if appraisee_answer != lines_number:
                    errors.update({answer_tag: question.constr_error_msg})
            if post['group'] == 'manager':
                if appraiser_answer != lines_number:
                    errors.update({answer_tag: question.constr_error_msg})
        return errors

    def validate_matrix_kits(self, cr, uid, question, post, answer_tag, context=None):
        errors = {}
        if question.constr_mandatory:
            lines_number = len(question.labels_ids_2)
            answer_candidates = dict_keys_startswith(post, answer_tag)
            ...
            for value in answer_candidates :
                ...
                if strip_value[3] == 'appraiser':
                    appraiser_answer = appraiser_answer + 1
            if post['kits_nb'] != '':
                if int(post['kits_nb']) != 0 :
                    if post['group'] == 'employee':
                        if appraisee_answer < int(post['kits_nb']):
                            errors.update({answer_tag: question.constr_error_msg})
                    if post['group'] == 'manager':
                        if appraiser_answer < int(post['kits_nb']):
                            errors.update({answer_tag: question.constr_error_msg})
        return errors

    def validate_kits_cycle_2(self, cr, uid, question, post, answer_tag, context=None):
        errors = {}
        if question.constr_mandatory:
            lines_number = len(question.labels_ids_2)
            answer_candidates = dict_keys_startswith(post, answer_tag)
            ...
            if answer_number != 0 :
                for value in answer_candidates :
                    if answer_candidates[value]!= '':
                        count = count + 1
                if count == 0:
                    errors.update({answer_tag: question.constr_error_msg})
        return errors

    def validate_matrix_monthly(self, cr, uid, question, post, answer_tag, context=None):
        errors = {}
        if question.constr_mandatory:
            lines_number = len(question.labels_ids_2)
            answer_candidates = dict_keys_startswith(post, answer_tag)
            ...
            for value in answer_candidates :
                ...
                if strip_value[3] ==  'appraisee':
                    if answer_candidates[value] != '':
                        appraisee_answer = appraisee_answer + 1
                if strip_value[3] ==  'comment':
                    if len(answer_candidates[value].split())!=0:
                    # if answer_candidates[value] != '':
                        appraisee_comment = appraisee_comment + 1
                if strip_value[3] == 'appraiser':
                    if answer_candidates[value] != '':
                        appraiser_answer = appraiser_answer + 1
                        
                # if strip_value[3] ==  'comment1':
                #     if answer_candidates[value] != '':
                #         appraiser_comment = appraiser_comment + 1

            if post['group'] == 'employee':
                if appraisee_answer + appraisee_comment != lines_number*2:
                    errors.update({answer_tag: question.constr_error_msg})
            if post['group'] == 'manager':
                if appraiser_answer != lines_number:
                    errors.update({answer_tag: question.constr_error_msg})
    ...
    def onchange_type(self, cr, uid, ids,type, context=None):
        vals = {}
        if type == 'matrix_kits':
            vals.update({'is_for_kit_cycle_1':True})
        if type == 'kits_cycle_2':
            vals.update({'is_for_kit_cycle_2':True})
        if type == 'matrix_monthly':
            vals.update({'is_for_monthly_appraisal':True})
        return {'value':vals}
...
class survey_label(osv.Model):
    ...
    _columns = {
        ...
        'question_id_2': fields.many2one('survey.question', 'Question',
            ondelete='cascade'),
        'question_id_3': fields.many2one('survey.question', 'Question',
            ondelete='cascade'),
        'sequence': fields.integer('Label Sequence order'),
        'value': fields.char("Suggested value", translate=True,
            required=True),
        # Adding Comment Box
        'appraisee_comment_box':fields.boolean("Appraisee Comments",help="Check this box to show Comments box for Appraisee"),
        'appraiser_comment_box':fields.boolean("Appraiser Comments",help="Check this box to show Comments box for Appraiser"),
        #######
    ...
    _defaults = {
        'sequence': 10,
        'appraisee_comment_box':True,
        'appraiser_comment_box':True,
    }
    _constraints = [
...
class survey_user_input(osv.Model):
    ...
    _columns = {
        ...
        'date_create': fields.datetime('Creation Date', required=True,
                                       readonly=1),
        'deadline': fields.datetime("Deadline/Appraisee Deadline",
                                help="Date by which the person can open the survey and submit answers",
                                oldname="date_deadline"),
        'deadline_2':fields.datetime("Deadline/Appraisee Deadline"),
...
class survey_user_input_line(osv.Model):
    ...
    _columns = {
        ...
        'is_appraiser':fields.boolean("Check if it is Appraiser"),
        'is_appraisee':fields.boolean("Check if it is Appraisee"),
        'user_id':fields.many2one('res.users', 'Answer Given by User'),
        'is_kits1':fields.boolean('Check if data is from kits cycle 1'),
        'is_kits2':fields.boolean('Check if data is from kits cycle 2'),
        'is_monthly_app':fields.boolean('Check if data is from Monthly Appraisal Form'),
        'previous_entry':fields.boolean("Is last Update"),
    }
    ...
    def save_line_free_text(self, cr, uid, user_input_id, question, post, answer_tag, context=None):
        ...
        hr_evaluation_interview_search = hr_evaluation_interview_obj.search(cr,uid,[('request_id','=',user_input_id)])
        hr_evaluation_interview_browse = hr_evaluation_interview_obj.browse(cr,uid,hr_evaluation_interview_search)
        if answer_tag in post and post[answer_tag].strip() != '':
            if post[answer_tag] != hr_evaluation_interview_browse.user_to_review_id.name:
                post[answer_tag] = hr_evaluation_interview_browse.user_to_review_id.name
        ...
        old_uil = self.search(cr, uid, [('user_input_id', '=', user_input_id),
                              ...
                              context=context)
        if old_uil:
            #vals.update({'answer_type': 'free_text', 'value_free_text': post[answer_tag],'skipped':False})
...
########## Logic for save line For Appraisal Form #############################
    def save_line_matrix_internal(self, cr, uid, user_input_id, question, post, answer_tag, context=None):
        vals = {
            'user_input_id': user_input_id,
            'question_id': question.id,
            'page_id': question.page_id.id,
        ...
        hr_evaluation_interview_search = hr_evaluation_interview_obj.search(cr,uid,[('request_id','=',user_input_id)])
        hr_evaluation_interview_browse = hr_evaluation_interview_obj.browse(cr,uid,hr_evaluation_interview_search)
        old_uil_appraisee = False
        old_uil_appraiser = False
        if uid == hr_evaluation_interview_browse.user_id.id :
            old_uil_appraisee = self.search(cr, uid, [('user_input_id', '=', user_input_id),
                                            ('survey_id', '=', question.survey_id.id),
                                            ('question_id', '=', question.id),
                                            ('is_appraisee','=',True)],
                                context=context)
        if uid == hr_evaluation_interview_browse.evaluation_id.employee_id.parent_id.user_id.id:
            old_uil_appraiser = self.search(cr, uid, [('user_input_id', '=', user_input_id),
                                            ('survey_id', '=', question.survey_id.id),
                                            ('question_id', '=', question.id),
                                            ('is_appraiser','=',True)],
                                context=context)
        ...
        if old_uil_appraisee:
            self.unlink(cr, uid, old_uil_appraisee, context=context)
        if old_uil_appraiser:
            self.unlink(cr, uid, old_uil_appraiser, context=context)

        no_answers = True
        ca = dict_keys_startswith(post, answer_tag)
        for row in question.labels_ids_2:
                a_tag = "%s_%s_%s" % (answer_tag, 'appraisee',row.id)
                a_tag_a = "%s_%s_%s" % (answer_tag,'appraiser',row.id)
        
                ###### Addind  Appraisee and Appraiser Comment Logic
                if uid == hr_evaluation_interview_browse.user_id.id:
                    appraisee_comment_answer = ca.pop(("%s_%s_%s" % (answer_tag, 'comment',row.id)), '').strip()
                    if appraisee_comment_answer:
                        vals.update({'answer_type': 'free_text', 'value_free_text': appraisee_comment_answer,'value_suggested_row': row.id,'quizz_mark':'', 'value_suggested': '','value_text':False,'is_appraisee':True,'is_appraiser':False})
                        self.create(cr, uid, vals, context=context)
                        no_answers = False
                if uid == hr_evaluation_interview_browse.evaluation_id.employee_id.parent_id.user_id.id:        
                    appraiser_comment_answer = ca.pop(("%s_%s_%s" % (answer_tag, 'comment1',row.id)), '').strip()
                    if appraiser_comment_answer:
                        vals.update({'answer_type': 'text', 'value_text': appraiser_comment_answer,'value_suggested_row': row.id,'quizz_mark':'', 'value_suggested': '','value_free_text':False,'is_appraiser':True,'is_appraisee':False})
                        self.create(cr, uid, vals, context=context)
                        no_answers = False
                ###########################################
                ...
                if a_tag in ca:
                    if uid == hr_evaluation_interview_browse.user_id.id:
                        no_answers = False
                        vals.update({'answer_type': 'suggestion', 'value_text':False, 'value_suggested': ca[a_tag], 'value_suggested_row': row.id,'quizz_mark':0,'value_free_text':False,'is_appraisee':True,'is_appraiser':False})
                        self.create(cr, uid, vals, context=context)
                if a_tag_a in ca:
                    if uid == hr_evaluation_interview_browse.evaluation_id.employee_id.parent_id.user_id.id:
                        no_answers = False
                        vals.update({'answer_type': 'suggestion', 'value_text':False, 'value_suggested': ca[a_tag_a], 'value_suggested_row': row.id,'quizz_mark':0,'value_free_text':False,'is_appraiser':True,'is_appraisee':False})
                        self.create(cr, uid, vals, context=context)
        ...
        if no_answers:
            ...
            self.create(cr, uid, vals, context=context)
        return True
##################################### End of Appraisal Form Logic ############################

############# Logic for save line For Kits Form for Cycle 1 #############################
    def save_line_matrix_kits(self, cr, uid, user_input_id, question, post, answer_tag, context=None):
        vals = {
            'user_input_id': user_input_id,
            'question_id': question.id,
            'page_id': question.page_id.id,
        ...
        for row in question.labels_ids_2:
                a_tag = "%s_%s_%s" % (answer_tag, 'appraisee',row.id)
                a_tag_a = "%s_%s_%s" % (answer_tag,'appraiser',row.id)
                a_tag_box = "%s_%s_%s" % (answer_tag,'kits1',row.id)
        
                ###### Addind  Appraisee and Appraiser Comment Logic
                if uid == hr_evaluation_interview_browse.user_id.id:
                    appraisee_comment_answer = ca.pop(("%s_%s_%s" % (answer_tag, 'comment',row.id)), '').strip()
                    if appraisee_comment_answer:
                        vals.update({'answer_type': 'free_text', 'value_free_text': appraisee_comment_answer,'value_suggested_row': row.id,'quizz_mark':'', 'value_suggested': '','value_text':False,'is_appraisee':True,'is_appraiser':False,'is_kits1':True})
                        self.create(cr, uid, vals, context=context)
                        no_answers = False
                if uid == hr_evaluation_interview_browse.evaluation_id.employee_id.parent_id.user_id.id:
                    appraiser_comment_answer = ca.pop(("%s_%s_%s" % (answer_tag, 'comment1',row.id)), '').strip()
                    if appraiser_comment_answer:
                        vals.update({'answer_type': 'text', 'value_text': appraiser_comment_answer,'value_suggested_row': row.id,'quizz_mark':'', 'value_suggested': '','value_free_text':False,'is_appraiser':True,'is_appraisee':False,'is_kits1':True})
                        self.create(cr, uid, vals, context=context)
                        no_answers = False
                ###########################################
                if a_tag_box in ca:
                    if ca[a_tag_box] != '' :
                        if uid == 1:
                            no_answers = False
                            vals.update({'answer_type': 'free_text', 'value_free_text': ca[a_tag_box],'value_suggested_row': row.id,'quizz_mark':'', 'value_suggested': '','value_text':False,'is_appraisee':False,'is_appraiser':False,'is_kits1':True})
                            self.create(cr, uid, vals, context=context)
                ...
                if a_tag in ca:
                    if ca[a_tag] != '' :
                        if uid == hr_evaluation_interview_browse.user_id.id:
                            no_answers = False
                            vals.update({'answer_type': 'suggestion', 'value_text':False, 'value_suggested': ca[a_tag], 'value_suggested_row': row.id,'quizz_mark':0,'value_free_text':False,'is_appraisee':True,'is_appraiser':False,'is_kits1':True})
                            self.create(cr, uid, vals, context=context)
                if a_tag_a in ca:
                    if ca[a_tag_a] != '' :
                        if uid == hr_evaluation_interview_browse.evaluation_id.employee_id.parent_id.user_id.id:
                            no_answers = False
                            vals.update({'answer_type': 'suggestion', 'value_text':False, 'value_suggested': ca[a_tag_a], 'value_suggested_row': row.id,'quizz_mark':0,'value_free_text':False,'is_appraiser':True,'is_appraisee':False,'is_kits1':True})
                            self.create(cr, uid, vals, context=context)
        ...
        if no_answers:
            ...
            self.create(cr, uid, vals, context=context)
        return True
##################################### End of Kits Form Logic for Cycle 1############################

############# Logic for save line For Kits Form for Cycle 2 #############################
    def save_line_kits_cycle_2(self, cr, uid, user_input_id, question, post, answer_tag, context=None):
        vals = {
            'user_input_id': user_input_id,
            'question_id': question.id,
            'page_id': question.page_id.id,
        ...
        ################ Finding Survey User Input Partner Id ################
        survey_user_input_obj = self.pool.get('survey.user_input')
        users_obj = self.pool.get('res.users')
        partner_id_survey = survey_user_input_obj.browse(cr,uid,[user_input_id])
        user_id_partner = users_obj.search(cr,uid,[('partner_id','=',partner_id_survey.partner_id.id),('active','=',True)])
        old_uil = False
        if uid == user_id_partner[0]:
            old_uil = self.search(cr, uid, [('user_input_id', '=', user_input_id),
                                            ('survey_id', '=', question.survey_id.id),
                                            ('question_id', '=', question.id),
                                            ('is_appraiser', '=', False),
        ...
        ca = dict_keys_startswith(post, answer_tag)
        for row in question.labels_ids_2:
                a_tag_box = "%s_%s_%s" % (answer_tag,'kits2',row.id)
        
                if a_tag_box in ca:
                    if ca[a_tag_box] != '' :
                        if uid == user_id_partner[0]:
                            no_answers = False
                            vals.update({'answer_type': 'free_text', 'value_free_text': ca[a_tag_box],'value_suggested_row': row.id,'quizz_mark':'', 'value_suggested': '','value_text':False,'is_appraisee':False,'is_appraiser':False,'is_kits2':True,'user_id':user_id_partner[0],'previous_entry':True})
                            self.create(cr, uid, vals, context=context)
        ...
        if no_answers:
            ...
            self.create(cr, uid, vals, context=context)
        return True
##################################### End of Kits Form Logic for Cycle 2############################

##################################### Logic For Monthly Appraisal Form############################
    def save_line_matrix_monthly(self, cr, uid, user_input_id, question, post, answer_tag, context=None):
        vals = {
            'user_input_id': user_input_id,
            'question_id': question.id,
            'page_id': question.page_id.id,
        ...
        user_object = self.pool.get('res.users')
        appraisal_rating_obj = self.pool.get('hr_evaluation.parameter.rating')
        hr_evaluation_interview_obj = self.pool.get('hr.evaluation.interview')
        hr_evaluation_obj = self.pool.get('hr_evaluation.evaluation')
        obj_survey_label = self.pool.get('survey.label')
        hr_evaluation_interview_search = hr_evaluation_interview_obj.search(cr,uid,[('request_id','=',user_input_id)])
        hr_evaluation_interview_browse = hr_evaluation_interview_obj.browse(cr,uid,hr_evaluation_interview_search)
        hr_evaluation_browse = hr_evaluation_obj.browse(cr, uid, hr_evaluation_interview_browse.evaluation_id.id)

        today = datetime.date.today()
        cycle1 = ['April', 'May', 'June','July', 'August', 'September']
        if today.strftime('%B') in cycle1:
            date_from = today.replace(today.year,4,1)
            date_to = today.replace(today.year,9,30)
        else:
            date_from = today.replace(today.year,10,1)
            date_to = today.replace(today.year+1,3,31)



        old_uil_appraisee = False
        old_uil_appraiser = False
        old_uil_hr_manager = False
        if uid == hr_evaluation_interview_browse.user_id.id:
            old_uil_appraisee = self.search(cr, uid, [('user_input_id', '=', user_input_id),
                                            ('survey_id', '=', question.survey_id.id),
                                            ('question_id', '=', question.id),
        ...
        if uid == hr_evaluation_interview_browse.evaluation_id.employee_id.parent_id.user_id.id:
            old_uil_appraiser = self.search(cr, uid, [('user_input_id', '=', user_input_id),
                                            ...
                                            ('is_appraiser','=',True)],
                                context=context)
        if self.pool.get('res.users').has_group(cr, uid, 'base.group_hr_user') and uid != hr_evaluation_interview_browse.evaluation_id.employee_id.parent_id.user_id.id:
            old_uil_hr_manager = self.search(cr, uid, [('user_input_id', '=', user_input_id),
                                            ('survey_id', '=', question.survey_id.id),
                                            ('question_id', '=', question.id)],
                                context=context)


        vals_for_appraisal_rating ={
            'evaluation_extends_id':hr_evaluation_interview_browse.evaluation_id.id,
            'interview_extends_id':hr_evaluation_interview_browse.id,
            'appraisal_plan_id':hr_evaluation_browse.plan_id.id,
            'cycle_type':hr_evaluation_interview_browse.cycle_type,
            'employee_id':hr_evaluation_interview_browse.user_to_review_id.id,
            'date_from':str(date_from),
            'date_to':str(date_to),
            'user_id':hr_evaluation_interview_browse.user_id.id,
            'date' : str(today),
            'manager': hr_evaluation_browse.manager.id,
            'company_id':hr_evaluation_browse.company_id.id,
        }
        if old_uil_appraisee:
            self.unlink(cr, uid, old_uil_appraisee, context=context)
        if old_uil_appraiser:
            self.unlink(cr, uid, old_uil_appraiser, context=context)
        if old_uil_hr_manager:
            self.unlink(cr, uid, old_uil_hr_manager, context=context)
        ...
        for row in question.labels_ids_2:
                ...
                ###### Addind  Appraisee and Appraiser Comment Logic
                if uid == hr_evaluation_interview_browse.user_id.id or self.pool.get('res.users').has_group(cr, uid, 'base.group_hr_user'):
                    appraisee_comment_answer = ca.pop(("%s_%s_%s" % (answer_tag, 'comment',row.id)), '').strip()
                    if appraisee_comment_answer:
                        vals.update({'answer_type': 'free_text', 'value_free_text': appraisee_comment_answer,'value_suggested_row': row.id,'quizz_mark':'', 'value_suggested': '','value_text':False,'is_appraisee':True,'is_appraiser':False,'is_monthly_app':True})
                        self.create(cr, uid, vals, context=context)
                        no_answers = False
                if uid == hr_evaluation_interview_browse.evaluation_id.employee_id.parent_id.user_id.id or self.pool.get('res.users').has_group(cr, uid, 'base.group_hr_user'):        
                    appraiser_comment_answer = ca.pop(("%s_%s_%s" % (answer_tag, 'comment1',row.id)), '').strip()
                    if appraiser_comment_answer:
                        vals.update({'answer_type': 'text', 'value_text': appraiser_comment_answer,'value_suggested_row': row.id,'quizz_mark':'', 'value_suggested': '','value_free_text':False,'is_appraiser':True,'is_appraisee':False,'is_monthly_app':True})
                        self.create(cr, uid, vals, context=context)
                        no_answers = False
                ###########################################
                ...
                if a_tag in ca:
                    if uid == hr_evaluation_interview_browse.user_id.id:
                        no_answers = False
                        if ca[a_tag] != '':
                            vals.update({'answer_type': 'suggestion', 'value_text':'', 'value_suggested': ca[a_tag], 'value_suggested_row': row.id,'quizz_mark':0,'value_free_text':'','is_appraisee':True,'is_appraiser':False,'is_monthly_app':True})
                            self.create(cr, uid, vals, context=context)
                            survey_label_value = obj_survey_label.browse(cr, uid, int(ca[a_tag])).value
                            survey_label_parameter_name = obj_survey_label.browse(cr, uid, row.id).value
                            if survey_label_value == 'NA':
                                survey_label_value = 0.0
                            vals_for_appraisal_rating.update({'name':survey_label_parameter_name,'appraisee_rating':survey_label_value,'appraisee_comment':appraisee_comment_answer})
                            ###############Unlink the existing rating record##############
                            rating_search=appraisal_rating_obj.search(cr, uid, [('interview_extends_id', '=', hr_evaluation_interview_browse.id),
                                            ('name', '=', survey_label_parameter_name)],context=context)
                            if rating_search != []:
                                appraisal_rating_obj.unlink(cr, uid, rating_search, context=context)
                            ################################
                            appraisal_rating_obj.create(cr, uid, vals_for_appraisal_rating)

                    elif self.pool.get('res.users').has_group(cr, uid, 'base.group_hr_user') and uid != hr_evaluation_interview_browse.user_id.id:
                        no_answers = False
                        if ca[a_tag] != '':
                            vals.update({'answer_type': 'suggestion', 'value_text':'', 'value_suggested': ca[a_tag], 'value_suggested_row': row.id,'quizz_mark':0,'value_free_text':'','is_appraisee':True,'is_appraiser':False,'is_monthly_app':True})
                            self.create(cr, uid, vals, context=context)
                ...
                if a_tag_a in ca:
                    if uid == hr_evaluation_interview_browse.evaluation_id.employee_id.parent_id.user_id.id:                       
                        no_answers = False
                        if ca[a_tag_a] != '':
                            vals.update({'answer_type': 'suggestion', 'value_text':'', 'value_suggested': ca[a_tag_a], 'value_suggested_row': row.id,'quizz_mark':0,'value_free_text':'','is_appraiser':True,'is_appraisee':False,'is_monthly_app':True})
                            self.create(cr, uid, vals, context=context)
                            survey_label_value = obj_survey_label.browse(cr, uid, int(ca[a_tag_a])).value
                            if survey_label_value == 'NA':
                                survey_label_value = 0.0
                            survey_label_parameter_name = obj_survey_label.browse(cr, uid, row.id).value
                            appraisal_rating_search = appraisal_rating_obj.search(cr, uid, [('interview_extends_id','=',hr_evaluation_interview_browse.id)])
                            for rating_record in appraisal_rating_obj.browse(cr,uid,appraisal_rating_search):
                                if rating_record.name == survey_label_parameter_name:
                                    appraisal_rating_obj.write(cr,uid,rating_record.id,{'appraiser_rating':survey_label_value,'appraiser_comment':appraiser_comment_answer})
        ...
        if no_answers:
            ...
            self.create(cr, uid, vals, context=context)
        return True
##################################### END OF Logic For Monthly Appraisal Form############################
