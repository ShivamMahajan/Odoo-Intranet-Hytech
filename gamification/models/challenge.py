...
class gamification_challenge(osv.Model):
    ...
    _columns = {
        ...
        'employee_ids': fields.many2many('hr.employee', 'gamification_challenge_employee_rel',
            string='Employees',
            help="List of User related Employee participating to the challenge"),
    ...
    def create(self, cr, uid, vals, context=None):
        ...
        if vals.get('user_domain'):
            ...
            employee_ids = self._get_challenger_employees(cr,uid,vals.get('user_domain'), context=context)

            if not vals.get('user_ids'):
                vals['user_ids'] = []
                vals['employee_ids'] = []
            vals['user_ids'] += [(4, user_id) for user_id in user_ids]
            vals['employee_ids'] += [(4, employee_id) for employee_id in employee_ids]
    ...
    #### New get List of Employee related to users #####
    def _get_challenger_employees(self, cr, uid, domain, context=None):
        user_domain = eval(ustr(domain))
        user_list = self.pool['res.users'].search(cr, uid, user_domain, context=context)
        return self.pool['hr.employee'].search(cr, uid, [('user_id','in',user_list)], context=context)
