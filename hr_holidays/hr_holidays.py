...
class hr_holidays_status(osv.osv):
    ...
    def get_days(self, cr, uid, ids, employee_id, context=None):
        ...
        holiday_ids = self.pool['hr.holidays'].search(cr, uid, [('employee_id', '=', employee_id),
                                                               ...
                                                               ('leave_allocate_year_end','<=', end_date)], context=context)
    ...
    _columns = {
        ...
        'leave_limit':fields.integer(string="Leave limit",help="Specify number of days/week/month is validate for this leave type"),
        'leave_limit_parameters':fields.selection([('days','Day'),('week','Week'),('month','Month')],string="Limit Measure"),
        'year_value':fields.float(string="Year After", help="Specify After how much year a employee can avail this leave"),
        'is_el':fields.boolean("Is EL"),
        'is_cl':fields.boolean("Is CL"),
        'is_unpaid':fields.boolean("Is Unpaid"),
        'company_ids':fields.many2many('res.company','company_rel', 'company_id', 'leave_id', string='Companies'),

    }
    _defaults = {
        'color_name': 'red',
        'active': True,
        'leave_limit_parameters':'days',
    ...
    def create(self, cr, uid, values, context=None):
        ...
        if values['is_el']:
            record=self.search(cr,uid,[('is_el','=',values['is_el'])])
            if record != []:
                raise Warning(_("Earned Leave type is already present"))
        if values['is_cl']:
            record=self.search(cr,uid,[('is_cl','=',values['is_cl'])])
            if record != []:
                raise Warning(_("Casual Leave type is already present"))
        if values['is_unpaid']:
            record=self.search(cr,uid,[('is_unpaid','=',values['is_unpaid'])])
            if record != []:
                raise Warning(_("Unpaid Leave type is already present"))

        return super(hr_holidays_status, self).create(cr, uid, values, context=context)

    def onchange_type_specifier(self, cr, uid, ids,is_el,is_cl,is_unpaid):
...
class hr_holidays(osv.osv):
    ...
    def _get_can_reset(self, cr, uid, ids, name, arg, context=None):
        """User can reset a leave request if it is its own leave request or ifmail_followers
        he is an Hr Manager. """
    ...
    _columns = {
        ...
        'state': fields.selection([('draft', 'To Submit'), ('cancel', 'Cancelled'),('cancel_approval','Waiting Cancel Approval'),('confirm', 'To Approve'), ('refuse', 'Refused'), ('validate1', 'Second Approval'), ('validate', 'Approved')],
        ...
        'holiday_status_id': fields.many2one("hr.holidays.status", "Leave Type", required=True,readonly=True, states={'draft':[('readonly',False)], 'confirm':[('readonly',False)],'validate1':[('readonly',False)]}),
        ...
        'manager_id': fields.many2one('hr.employee', 'Manager', invisible=False, readonly=False, copy=False,
    ...
    def write(self, cr, uid, ids, vals, context=None):
        #if vals.get('state') and vals['state'] not in ['draft', 'confirm', 'cancel'] and not self.pool['res.users'].has_group(cr, uid, 'base.group_hr_project_manager'):
    ...
    def holidays_validate(self, cr, uid, ids, context=None):
        ...
        for record in data_holiday:
            if record.double_validation and self.pool['res.users'].has_group(cr, uid, 'base.group_hr_manager'):
                self.write(cr, uid, [record.id], {'manager_id2': manager})
            else:
               # self.write(cr, uid, [record.id], {'manager_id': manager})
               raise osv.except_osv(_('Warning!'),_('Only Access right with HR manager will able to do Second Approval'))
        ...
        for record in self.browse(cr, uid, ids, context=context):
            ...
            if leave_days['remaining_leaves'] < 0: #or leave_days['virtual_remaining_leaves'] < 0:
                ...
                raise Warning(_('The number of remaining leaves is not sufficient for this leave type.\n'
                                ...
                                'Your Correct remaining leave is reflect when all your previous leave is validated'))
        return True
    ...
    def holidays_first_validate_notificate(self, cr, uid, ids, context=None):
        for obj in self.browse(cr, uid, ids, context=context):
            self.message_post(cr, uid, [obj.id],
                _("Request approved, waiting second validation. \n\n Employee Name: %s and Applied for %s to %s ")%(obj.employee_id.name,obj.date_from,obj.date_to), context=context)
