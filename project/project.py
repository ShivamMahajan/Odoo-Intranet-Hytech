    ...
    def onchange_project(self, cr, uid, id, project_id, context=None):
        if project_id:
            ...
            for m in project.members:
                members.append(m.id)
            if project and project.partner_id:
                return {'value': {'partner_id': project.partner_id.id},'domain': {'user_id':[('id','in',members)]}}
...
class project_work(osv.osv):
    ...
    _columns = {
        ...
        'user_id': fields.many2one('res.users', 'Done by', readonly=True, required=True, select="1",),
...
class account_analytic_account(osv.osv):
    ...
    def project_create(self, cr, uid, analytic_account_id, vals, context=None):
        ...
        if not project_id and self._trigger_project_creation(cr, uid, vals, context=context):
            ...
            #print("Project id in analytic"),project_pool.create(cr, uid, project_values, context=context)
