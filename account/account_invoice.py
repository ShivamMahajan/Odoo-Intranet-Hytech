...
class account_invoice(models.Model):
    ...
    state = fields.Selection([
            ...
            ('proforma','Pro-forma'),
            ('proforma2','Pro-forma'),
            ('approved_for_account','Approved For Account'),
            ('open','Open'),
            ('paid','Paid'),
            ('cancel','Cancelled'),
    ...
    engagement_type = fields.Selection([ ('fixed_budgets','Fixed Budgets'), ('time_materials','Time and Materials'),('resources','Resources') ],'Engagement Type', select=True, required=True, default='fixed_budgets', help="Select Enagement Type to define type of project")
    employee_id = fields.Many2one('hr.employee', string='Salesperson Related Employee')
    actual_bill_date = fields.Date('Actual Bill Date')
    actual_bill_no = fields.Char('Billed Invoice Number')
    ...
    def create(self,vals):
        ...
        sub = "Invoice created for %s"%(invoice_id.partner_id.name)
        body_html = '''<html>
                            <head></head>
                                <body>
                                    <p> Hi,</p>
                                    <p> <b> Invoice Information are:-</b> </p>
                                    <p> <b> Customer Name :- %s </b> </p>
                                    <p> <b> Amount :- %s</b> </p>
                                    <p> <b> Date Created :- %s </b> </p>
                                </body>
                    </html> ''' %(invoice_id.partner_id.name, invoice_id.amount_total, str(today))
        mail_vals = {'state': 'outgoing',
                    'subject': sub,
                    'body_html': '<pre>%s</pre>' % body_html,
                    'email_to': invoice_id.user_id.login,
                    'email_from': 'hytechproerp@gmail.com' ,
                }
    ...
    @api.multi
    def approved_for_account_open(self):
        return self.write({'state': 'approved_for_account'})
    ...
    def on_change_user(self, cr, uid, ids, user_id, context=None):
        """ When changing the user, fetching employee_id """
        ######################Fetch  Employee ID of this user#####################
        hr_employee_obj=self.pool.get('hr.employee')
        hr_employee_search= hr_employee_obj.search(cr,uid,[('user_id','=',user_id),('active','=',True)],context=context)
        hr_employee_browse = hr_employee_obj.browse(cr,uid,hr_employee_search,context=context)
        ################################
        return {'value': {'employee_id':hr_employee_browse.id}}
    ...
    def invoice_validate(self):
        return self.write({'state': 'open','actual_bill_date':fields.Date.context_today(self)})
...
#############Cron Job For Mailing Required Invoice exception############
    def run_followup_for_partner_invoice(self, cr, uid, automatic=False, use_new_cursor=False, context=None):
        today = datetime.today()
        today_date_only = today.strftime("%Y-%m-%d")
        invoice_record = self.search(cr,uid,[('date_due','<=',today_date_only),('type','=','out_invoice'),('state','in',['draft','open'])])
        add_row_two_days = ''
        add_row_three_more_days = ''

        invoice_browse_customer = []
        for customer_record in self.browse(cr,uid,invoice_record):
            if customer_record.partner_id.id not in invoice_browse_customer:
                invoice_browse_customer.append(customer_record.partner_id.id)

        for customer_list in invoice_browse_customer:
            number_of_record_search = self.search(cr,uid,[('partner_id','=',customer_list)])
            if len(number_of_record_search) <= 2:
                total_amount_two_days = 0.0
                balance_amount_two_days = 0.0
                count_two_days = 0
                for two_days_invoices in self.browse(cr,uid,number_of_record_search):
                    total_amount_two_days += two_days_invoices.amount_total
                    balance_amount_two_days += two_days_invoices.residual
                    count_two_days +=1
                add_row_two_days += '''<tr bgcolor="#d7e9fa"><td> %s </td> <td style="border-right: 1px solid #ccc;text-align: center;"> %s </td> <td style="border-right: 1px solid #ccc;text-align: center;"> %s </td> <td style="border-right: 1px solid #ccc;text-align: center;"> %s </td> </tr>''' %(two_days_invoices.partner_id.name,total_amount_two_days,count_two_days,balance_amount_two_days)
            if len(number_of_record_search) >= 3:
                total_amount_three_days = 0.0
                balance_amount_three_days = 0.0
                count_three_days = 0
                for three_days_invoices in self.browse(cr,uid,number_of_record_search):
                    total_amount_three_days += three_days_invoices.amount_total
                    balance_amount_three_days += three_days_invoices.residual
                    count_three_days +=1
                add_row_three_more_days += '''<tr bgcolor="#d7e9fa"><td> %s </td> <td style="border-right: 1px solid #ccc;text-align: center;"> %s </td> <td style="border-right: 1px solid #ccc;text-align: center;"> %s </td> <td style="border-right: 1px solid #ccc;text-align: center;"> %s </td></tr>''' %(three_days_invoices.partner_id.name,total_amount_three_days,count_three_days,balance_amount_three_days)

        body_html = '''<html><head></head><body><p> Hi,</p><p> Below are the Invoice Exception Log Based on Due Date</p> 
                        
                            <table>
                            <caption><b>Three or more than three Invoice Record Pending For Further Process</b></caption>
                                <thead>
                                    <tr bgcolor="#f2f3f2">
                                        <th style="width:24%%">Customer Name </th>
                                        <th style="border-right: 1px solid #ccc;"> Total Amount </th>
                                        <th style="border-right: 1px solid #ccc;"> No. of Invoices</th>
                                        <th style="border-right: 1px solid #ccc;"> Balance Amount</th>
                                    </tr>
                                </thead>
                                <tbody>
                                        %s
        ...
        if today.strftime("%a") == 'Mon':
            body_html = '''<html><head></head><body><p> Hi,</p><p> Below are the Invoice Exception Log Based on Due Date</p> 
                            <table>
                            <caption><b>Two Invoice Record Pending For Further Process</b></caption>
                                <thead>
                                    <tr bgcolor="#f2f3f2">
                                        <th style="width:24%%">Customer Name </th>
                                        <th style="border-right: 1px solid #ccc;"> Total Amount </th>
                        ...
                        ''' %(add_row_two_days,add_row_three_more_days)
        
        sub = 'Invoice Report Logs' 
        ...
        email_to = irconfig_obj.get_param(cr, SUPERUSER_ID, 'partner.invoice.exception.email_to')
        vals = {'state': 'outgoing',
            'subject': sub,
            'body_html': '<pre>%s</pre>' % body_html,
            'email_to': email_to,
